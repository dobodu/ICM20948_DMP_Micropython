...
ICM_USER_CTRL_DMP_EN = 0x80
ICM_USER_CTRL_FIFO_EN = 0x40
ICM_USER_CTRL_I2C_MST_EN = 0x20
ICM_USER_CTRL_I2C_IF_DIS = 0x10
ICM_USER_CTRL_DMP_RST = 0x08
ICM_USER_CTRL_SRAM_RST = 0x04
...
ICM_LP_CFG = 0x05
ICM_LP_CFG_GYRO = 0b00001000
ICM_LP_CFG_ACC = 0b00000100
ICM_LP_CFG_MST =0b00000010
ICM_PWR_MGMT_1 = 0x06
...
#BANK 3 COMMON
ICM_I2C_SLV_ADDR_READ = 0x80
ICM_I2C_SLV_CTRL_SLV_EN = 0x80
...
#DMP Header Bitmap (from highest bit to lowest bit)
...
DMP_header_bitmap_Pedom_Quat6 = 0x0200
...
DMP_header_bitmap_Step_Ind_2 = 0x0004
DMP_header_bitmap_Step_Ind_1 = 0x0002
DMP_header_bitmap_Step_Ind_0 = 0x0001

#DMP_Header2_Bitmap (from highest bit to lowest bit)
...
DMP_header2_bitmap_Batch_Mode_Enable = 0x0100
...
DMP_Data_Output_Control_1_Accel = 0x8000 #16 bit
DMP_Data_Output_Control_1_Gyro = 0x4000 #16 bit
DMP_Data_Output_Control_1_Compass = 0x2000 #16 bit
DMP_Data_Output_Control_1_ALS = 0x1000 #16 bit
DMP_Data_Output_Control_1_Quat6 = 0x0800 #32 bit 6 axis
DMP_Data_Output_Control_1_Quat9 = 0x0400 #32 bit 6 axis
DMP_Data_Output_Control_1_Pedom_Quat6 = 0x0200 #16 bit
DMP_Data_Output_Control_1_Geomag = 0x0100 # 32 bit + heading accuracy
DMP_Data_Output_Control_1_Pressure = 0x0080 #16 bit
DMP_Data_Output_Control_1_Gyro_Calibr = 0x0040 #32 bit
DMP_Data_Output_Control_1_Compass_Calibr = 0x0020 #32 bit
...
ANDROID_SENSORS_CTRL_BITS = { 0 : 0xFFFF, 1 : 0x8008, 2 : 0x0028, 3 : 0x0408,
    ...
    40 : 0xFFFF, 41 : 0x0000, 42 : 0x8008, 43 : 0x4048, 44 : 0xFDF8}
...
class ICM20948:
    ...
    #Initialise the IMU
    def __init__(self, i2c=None, addr=None, debug=False):
        ...
        self._fifo_buffer = bytearray(1024)
        ...
        #Check if the Chip ID is the right one   
        self.bank(0)
        ...
        self.reset() 
        sleep_ms(10)
        self.write(ICM_PWR_MGMT_1, 0x01)  #0x01 = CLK selector 1 = Autoselect
        self.write(ICM_PWR_MGMT_2, 0x00)  #0x00 = ACC and GYO are ON

        #Configure scale, Low Pass and Sample rate, and read sensitivity values
        self.bank(2)
        ...
        self.bank(3)
        #self.write(ICM_I2C_MST_CTRL, 0x4D)  #0b0100 1101 = I2C MSTR CLOCK = 15 = 345,6kHz
    ...
    def set_acc_sample_rate(self, rate=125):
        ...
        self.write(ICM_ACCEL_SMPLRT_DIV_1, (rate >> 8) & 0xff)
...
#===========Below are all internal communication functions ===================================
    ...
    def reset(self):
        self.bank(0)
        user = self.read(ICM_PWR_MGMT_1)
        self.write(ICM_PWR_MGMT_1, user | ICM_PWR_MGMT_1_RESET)
        sleep_ms(10)

    #Set ICM20948 in sleep mode
    def sleep(self, enable=True):
        self.bank(0)
        user = self.read(ICM_PWR_MGMT_1)
        if enable :
            user &= ICM_PWR_MGMT_1_SLEEP
        else :
            user &= ~ICM_PWR_MGMT_1_SLEEP
        self.write(ICM_PWR_MGMT_1, user)
        sleep_ms(10)
        
    #Set ICM20948 in low power mode
    def low_power(self, enable=True):
        self.bank(0)
        user = self.read(ICM_PWR_MGMT_1)
        if enable :
            user &= ICM_PWR_MGMT_1_LP
        else :
            user &= ~ICM_PWR_MGMT_1_LP
        self.write(ICM_PWR_MGMT_1, user)
        sleep_ms(10)
        
    #Configure ICM20948 low power mode for lp_sensor
    def low_power_config(self, lp_sensor, enable=True):
        self.bank(0)
        user = self.read(ICM_LP_CFG)
        if enable :
            user &= lp_sensor
        else :
            user &= ~lp_sensor
    ...
    def user_ctrl_config(self, ctrl, enable=True):
        self.bank(0)
        user = self.read(ICM_USER_CTRL)
        #adjust user_ctrl if enabled or disabled
        if enable :
            user |= ctrl
        else :
            user &= ~ctrl
    ...
    def I2C_master_setmag(self):
        ...
        self.bank(3)
        self.write(ICM_I2C_SLV0_CTRL, ICM_I2C_SLV_CTRL_SLV_EN | 8) # 0x80 = I2C_SLV0_EN = Enable reading data, 1 = for 1 byte
    ...
    def mag_read(self, reg):
        self.bank(3)
        self.write(ICM_I2C_SLV0_CTRL, ICM_I2C_SLV_CTRL_SLV_EN | 1) # 0x80 = I2C_SLV0_EN = Enable reading data, 1 = for 1 byte
        self.write(ICM_I2C_SLV0_ADDR, AK_I2C_ADDR | ICM_I2C_SLV_ADDR_READ)  #0x80 = Transfer is READ
    ...
    def mag_read_bytes(self, reg, length=1):
        self.bank(3)     
        self.write(ICM_I2C_SLV0_CTRL, ICM_I2C_SLV_CTRL_SLV_EN | 0x08 | length)  #0x80 = Enable reading data,  0x08 = Grouped by 2
        self.write(ICM_I2C_SLV0_ADDR, AK_I2C_ADDR | ICM_I2C_SLV_ADDR_READ) #0x80 = Transfer is READ
        self.write(ICM_I2C_SLV0_REG, reg)
...
#=================== Below are all DMP related functions ==============================            
        ...
        while data_pos < len(dmp_img):
            ...
            if not burstmode :
                for d in data:
                    ...
                    self._bus.writeto_mem(self._addr, ICM_MEM_START_ADDR, self._buffer_1)
                    self._buffer_1[0] = d
                    self._bus.writeto_mem(self._addr, ICM_MEM_R_W, self._buffer_1)
                    address += 1    
            #Write firmware in burst mode (up to 256 byte at a time : Damn fast)
            else :
                self._buffer_1[0] = address
                self._bus.writeto_mem(self._addr, ICM_MEM_START_ADDR, self._buffer_1)
                self._bus.writeto_mem(self._addr, ICM_MEM_R_W, bytes(data))
    ...
    def DMP_config(self):
        self.bank(3)
        #SLV0 will read 10 byte AK09916 RSV register
        self.write(ICM_I2C_SLV0_CTRL, ICM_I2C_SLV_CTRL_SLV_EN | 8) # 0x80 = I2C_SLV0_EN = Enable reading data, 1 = for 1 byte
        self.write(ICM_I2C_SLV0_ADDR, AK_I2C_ADDR | 0x80)  #0x80 = Transfer is READ
        self.write(ICM_I2C_SLV0_REG, AK_RSV2) #Read from Reserved register
        #SLV1 will do single measurement triggering
        self.write(ICM_I2C_SLV1_CTRL, ICM_I2C_SLV_CTRL_SLV_EN | 8) # 0x80 = I2C_SLV0_EN = Enable reading data, 1 = for 1 byte
        self.write(ICM_I2C_SLV1_ADDR, AK_I2C_ADDR | 0x80)  #0x80 = Transfer is READ
        self.write(ICM_I2C_SLV1_REG, AK_CNTL2) #Read from Reserved register
        #Configure ODR to 68,75 Hz
        self.write(ICM_I2C_MST_ODR_CONFIG, 0x04) # 4 because 1100/2**4 = 68.75 Hz
        #Configure clock source
        self.bank(0)
        self.write(ICM_PWR_MGMT_1, 0x01)  #0x01 = CLK selector 1 = Autoselect
        self.write(ICM_PWR_MGMT_2, 0x00)  #0x00 = ACC and GYO are ON
        #Place I2C_master only in Low Power Mode
        self.low_power_config(ICM_LP_CFG_MST, True)
        #Disable FIFO
        self.user_ctrl_config(ICM_USER_CTRL_FIFO_EN, False)
        #Disable DMP
        self.user_ctrl_config(ICM_USER_CTRL_DMP_EN, False)
        ...
        #Turn Off data ready interrupt  1576
        #Todo
        #Reset the Fifo
        self.DMP_fifo_reset()
        #Set Accelerator Sample_rate 55 Hz
        self.set_acc_sample_rate(55)
        #Set Accelerator Sample_rate 55Hz
        ...
        self.bank(2)
        self._bus.writeto_mem(self._addr, ICM_PRGM_START_ADDRH, self._buffer_n)

        #Set the Hardware Fix Disable register to 0x48
        self.bank(0)
        self._buffer_1[0] = 0x48
        self._bus.writeto_mem(self._addr, ICM_HW_FIX_DISABLE, self._buffer_1)

        #Set the Single FIFO Priority Select register to 0xE4
        self.bank(0)
        self._buffer_1[0] = 0xE4
        self._bus.writeto_mem(self._addr, ICM_SINGLE_FIFO_PRIORITY_SEL, self._buffer_1)
        
        #Configure Acceleration in order to align acc_raw_data = 2^25 = 1g when FSR = 4g
        self.DMP_set_acc_full_scale(scale=4)
        ...
        DMP_ACCEL_CAL_RATE_FACTOR = bytearray([0x00, 0x00, 0x00, 0x00])
        ...
        self.bank(0)
        self.write(ICM_USER_CTRL, ICM_USER_CTRL_DMP_EN | ICM_USER_CTRL_FIFO_EN | ICM_USER_CTRL_DMP_RST | ICM_USER_CTRL_SRAM_RST)
        sleep_ms(10)
    ...
    #Reset fifo
    def DMP_fifo_reset(self):
        self.bank(0)
        data = self.read(ICM_FIFO_RST)
        data = 0x1F # Why 5 bit while Datasheet says 4 bits... ?
        self.write(ICM_FIFO_RST,data)
        sleep_ms(10)
        data = 0x1E # Why, copied from Sparkfun, copied from Nucleo example
        self.write(ICM_FIFO_RST,data)
        sleep_ms(10)
    ...
    def DMP_fifo_count(self):
        ...
        data[0] &= 0x1F
        f = unpack_from(">h", data)[0]
        #f &= 0x1FFF #Fifo_H is 12:8 and Fifo_L is 7:0
        return f
    
    #Upload Fifo bytes
    def DMP_fifo_read(self, size):
        self.bank(0)
        self._dbg("Processing FIFO Bytes ({:.0f})".format(size))
        header = 0
        for i in range(DMP_Header_Bytes) :  #Read header (2 Bytes)
            data = self.read(ICM_FIFO_R_W)
            header |= data << (8 - (i * 8))
        size -= DMP_Header_Bytes  #Decrease size of Header size
        self._dbg("Header is",hex(header))
        header2 = 0
        if (header & DMP_header_bitmap_Header2) != 0 : #Check if header contains header 2 bit
            self._dbg("Header_2 detected")
            #Here we should check if we have enought data in fifo to read header2
            for i in range(DMP_header2_Bytes) :  #Read header 2 (2 Bytes)
                data = self.read(ICM_FIFO_R_W)
                header2 |= data << (8 - (i * 8))
            size -= DMP_header2_Bytes  #Decrease size of Header 2 size

        #Check bit per bit header
        #Acceleration
        if (header & DMP_header_bitmap_Accel) != 0 :
            self._dbg("Fifo acceleration data detected")
            ...
            ay *= self._acc_s
            az *= self._acc_s
            self._dbg("Fifo acceleration ax", ax,"\tay",ay, "\taz",az)
            size -= DMP_Raw_Accel_Bytes  #Decrease size of Acc
            
        #Gyroscope
        if (header & DMP_header_bitmap_Gyro) != 0 :
            self._dbg("Fifo gyroscope data detected")
        ...
        if (header & DMP_header_bitmap_Compass) != 0 :
            self._dbg("Fifo magnetometer data detected")
        ...
        if (header & DMP_header_bitmap_ALS) != 0 :
            self._dbg("Fifo ALS data detected")
        ...
        if (header & DMP_header_bitmap_Quat6) != 0 :
            self._dbg("Fifo Quaternion 6 data detected")
            ...
            q3 /= 2**30
            #To do process
            self._dbg("Fifo Quaternion 6 q1", q1,"\tq2", q2, "\tq3", q3)
            size -= DMP_Quat6_Bytes
            
        #Quaternion 9
        if (header & DMP_header_bitmap_Quat9) != 0 :
            self._dbg("Fifo Quaternion 9 data detected")
        ...
        #PQuaternion 6
        if (header & DMP_header_bitmap_Pedom_Quat6) != 0 :
            self._dbg("Fifo PQuaternion 6 data detected")
            ...
            q3 /= 2**30
            #To do process
            self._dbg("Fifo P-Quaternion 6 q1", q1,"\tq2", q2, "\tq3", q3)
            size -= DMP_Pedom_Quat6_Bytes

        #Geomag
        if (header & DMP_header_bitmap_Geomag) != 0 :
            self._dbg("Fifo Geomag data detected")
            ...
            q2 /= 2**30
            q3 /= 2**30
            self._dbg("Fifo Geomag Quaternion q1", q1,"\tq2", q2, "\tq3", q3, "\accuracy", acc)
            size -= DMP_Geomag_Bytes
            
        #Pressure
        if (header & DMP_header_bitmap_Pressure) != 0 :
            self._dbg("Fifo Pressure data detected")
            data = self.read_bytes(ICM_FIFO_R_W, DMP_Pressure_Bytes)
            press = data[0] | (data[1] << 8) | (data[2] << 16)
            temp = data[3] | (data[4] << 8) | (data[5] << 16)
            self._dbg("Fifo Pressure", press, "\tTemperature",temp)
            size -= DMP_Pressure_Bytes
            
        #Gyro calibration 
        if (header & DMP_header_bitmap_Gyro_Calibr) != 0 :
            self._dbg("Fifo Gyro calibration data detected")
            data = self.read_bytes(ICM_FIFO_R_W, DMP_Gyro_Calibr_Bytes)
            #To do process
            self._dbg("Fifo Gyro calibration", data)
            size -= DMP_Gyro_Calibr_Bytes
        #Compass calibration 
        if (header & DMP_header_bitmap_Compass_Calibr) != 0 :
            self._dbg("Fifo Compass calibration data detected")
            data = self.read_bytes(ICM_FIFO_R_W, DMP_Compass_Calibr_Bytes)
            #To do process
            self._dbg("Fifo Compass calibration", data)
            size -= DMP_Compass_Calibr_Bytes
        #step
        if (header & DMP_header_bitmap_Step_Detector) != 0 :
            self._dbg("Fifo step detector data detected")
            data = self.read_bytes(ICM_FIFO_R_W, DMP_Step_Detector_Bytes)
            #To do process
            self._dbg("Fifo step detector data", data)
            size -= DMP_Step_Detector_Bytes
        #Check bit per bit header2
        #Accelerometer Accuracy
        if (header2 & DMP_header2_bitmap_Accel_Accuracy) != 0 :
            self._dbg("Fifo Accel Accuracy data detected")
            data = self.read_bytes(ICM_FIFO_R_W, DMP_Accel_Accuracy_Bytes)
            #To do process
            self._dbg("Fifo Accel Accuracy data", data)
            size -= DMP_Accel_Accuracy_Bytes
        #Gyro Accuracy
        if (header2 & DMP_header2_bitmap_Gyro_Accuracy) != 0 :
            self._dbg("Fifo Accel Accuracy data detected")
            data = self.read_bytes(ICM_FIFO_R_W, DMP_Gyro_Accuracy_Bytes)
            #To do process
            self._dbg("Fifo Accel Accuracy data", data)
            size -= DMP_Gyro_Accuracy_Bytes
        #Compass Accuracy
        if (header2 & DMP_header2_bitmap_Compass_Accuracy) != 0 :
            self._dbg("Fifo Compass Accuracy data detected")
            data = self.read_bytes(ICM_FIFO_R_W, DMP_Compass_Accuracy_Bytes)
            #To do process
            self._dbg("Fifo Compass Accuracy data", data)
            size -= DMP_Compass_Accuracy_Bytes        
        #Fsynch Detection
        if (header2 & DMP_header2_bitmap_Fsync) != 0 :
            self._dbg("Fifo Fsynch Detection data detected")
            data = self.read_bytes(ICM_FIFO_R_W, DMP_Fsync_Detection_Bytes)
            #To do process
            self._dbg("Fifo Fsynch Detection data", data)
            size -= DMP_Fsync_Detection_Bytes
        #Pickup
        if (header2 & DMP_header2_bitmap_Pickup) != 0 :
            self._dbg("Fifo Pickup data detected")
            data = self.read_bytes(ICM_FIFO_R_W, DMP_Pickup_Bytes)
            #To do process
            self._dbg("Fifo Pickup data", data)
            size -= DMP_Pickup_Bytes 
        #Activity recog
        if (header2 & DMP_header2_bitmap_Activity_Recog) != 0 :
            self._dbg("Fifo Pickup data detected")
            data = self.read_bytes(ICM_FIFO_R_W, DMP_Activity_Recognition_Bytes)
            #To do process
            self._dbg("Fifo Pickup data", data)
            size -= DMP_Activity_Recognition_Bytes
        #Secondary On Off
        if (header2 & DMP_header2_bitmap_Secondary_On_Off) != 0 :
            self._dbg("Fifo Secondary On Off data detected")
            data = self.read_bytes(ICM_FIFO_R_W, DMP_Secondary_On_Off_Bytes)
            #To do process
            self._dbg("Fifo Secondary On Off data", data)
            size -= DMP_Secondary_On_Off_Bytes            
        #Finally process the footer
        footer = 0
        for i in range(DMP_Footer_Bytes) :  #Read header (2 Bytes)
            data = self.read(ICM_FIFO_R_W)
            footer |= data << (8 - (i * 8))
        size -= DMP_Footer_Bytes 
    ...
    def DMP_write(self, register, data):
        ...
        self._buffer_1[0] = reg_address
        self._bus.writeto_mem(self._addr, ICM_MEM_START_ADDR, self._buffer_1)
        self._bus.writeto_mem(self._addr, ICM_MEM_R_W, data)
    ...
    def DMP_activate_sensor(self, icm_sensor, enable=True):
        ...
        if ((_data_out_ctl & DMP_Data_Output_Control_1_Gyro) | (_data_out_ctl & DMP_Data_Output_Control_1_Gyro_Calibr) > 0) :
            _data_out_ctl2 |= DMP_Data_Output_Control_2_Gyro_Accuracy
        if ((_data_out_ctl & DMP_Data_Output_Control_1_Compass_Calibr) | (_data_out_ctl & DMP_Data_Output_Control_1_Compass) | (_data_out_ctl & DMP_Data_Output_Control_1_Quat9) | (_data_out_ctl & DMP_Data_Output_Control_1_Geomag) > 0) :
            _data_out_ctl2 |= DMP_Data_Output_Control_2_Compass_Accuracy
        #Reconstruc INV_EVENT_CTRL
        if ((_data_out_ctl & DMP_Data_Output_Control_1_Quat9 ) > 0) :
            _inv_event_ctl |= DMP_Motion_Event_Control_9axis
        if ((_data_out_ctl & DMP_Data_Output_Control_1_Geomag) > 0) :
            _inv_event_ctl |= DMP_Motion_Event_Control_Geomag
        if ((_data_out_ctl & DMP_Data_Output_Control_1_Step_Detector) | (_data_out_ctl & DMP_Data_Output_Control_1_Step_Ind_0) | (_data_out_ctl & DMP_Data_Output_Control_1_Step_Ind_1) | (_data_out_ctl & DMP_Data_Output_Control_1_Step_Ind_2) > 0) :
